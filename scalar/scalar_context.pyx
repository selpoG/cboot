...
class k_poleData:
    ...
    def coeff(self):
        if self.k % 2:
            ...
            local_sign = 1
        return (-local_sign * (self.k) / (2 * self.context.pochhammer(1,
                                                                      self.k) ** 2)) * self.context.pochhammer((1 - self.k + 2 * self.a) / 2,
                                                                                                               self.k) * self.context.pochhammer((1 - self.k + 2 * self.b) / 2,
                                                                                                                                                 self.k)

    def residue_of_h(self):
        return self.coeff() * self.context.chiral_h_times_rho_to_n(self.descendant_level(),
                                                                   self.residueDelta(), -2 * self.a, 2 * self.b)
...
class k_rational_approx_data:
    ...
    def __init__(self, context, cutoff, Delta_1_2, Delta_3_4,
        ...
        if approximate_poles:
            ...
            self.approx_column = lambda x: [1 / (unitarity_bound - x.polePosition()) ** i for i in range(
                1, dim_approx_base // 2 + 2)] + [(x.polePosition()) ** i for i in range(0, (dim_approx_base + 1) // 2 - 1)]
            ...
            if is_correlator_multiple:
                self.approx_poles = [
                    k_poleData(
                        ...
                        self.a,
                        self.b,
                        context) for x in range(
                        cutoff + 1,
                        2 * cutoff + 1)]
            else:
                self.approx_poles = [k_poleData(x, self.a, self.b, context) for x in range(
                    2 * (cutoff // 2) + 2, 2 * cutoff + 2, 2)]
    ...
    def approx_chiral_h(self):
        ...
        _polys = [reduce(lambda y, z:y * z, [(self.context.Delta - w.polePosition())
                                             for w in self.poles if not w == x]) for x in self.poles]
...
class poleData:
    ...
    def coeff(self):
        if self.k % 2:
            ...
            local_sign = 1
        if self.type == 1:
            return (-local_sign * (self.k) / (self.context.pochhammer(1,
                                                                      self.k) ** 2)) * self.context.pochhammer(self.ell + 2 * self.epsilon,
                                                                                                               self.k) * self.context.pochhammer((1 - self.k + 2 * self.a) / 2,
                                                                                                                                                 self.k) * self.context.pochhammer((1 - self.k + 2 * self.b) / 2,
                                                                                                                                                                                   self.k) / (self.context.pochhammer(self.ell + self.epsilon,
                                                                                                                                                                                                                      self.k))
        elif self.type == 2:
            return (-local_sign * (self.k) / (self.context.pochhammer(1,
                                                                      self.k) ** 2)) * self.context.pochhammer(self.epsilon - self.k,
                                                                                                               2 * self.k) / (self.context.pochhammer(self.ell + self.epsilon - self.k,
                                                                                                                                                      2 * self.k) * self.context.pochhammer(self.ell + self.epsilon + 1 - self.k,
                                                                                                                                                                                            2 * self.k)) * self.context.pochhammer((1 - self.k + self.ell - 2 * self.a + self.epsilon) / 2,
                                                                                                                                                                                                                                   self.k) * self.context.pochhammer((1 - self.k + self.ell + 2 * self.a + self.epsilon) / 2,
                                                                                                                                                                                                                                                                     self.k) * self.context.pochhammer((1 - self.k + self.ell - 2 * self.b + self.epsilon) / 2,
                                                                                                                                                                                                                                                                                                       self.k) * self.context.pochhammer((1 - self.k + self.ell + 2 * self.b + self.epsilon) / 2,
                                                                                                                                                                                                                                                                                                                                         self.k)
            ...
            if self.k <= self.ell:
                return (-local_sign * (self.k) / (self.context.pochhammer(1,
                                                                          self.k) ** 2)) * self.context.pochhammer(self.ell + 1 - self.k,
                                                                                                                   self.k) * self.context.pochhammer((1 - self.k + 2 * self.a) / 2,
                                                                                                                                                     self.k) * self.context.pochhammer((1 - self.k + 2 * self.b) / 2,
                                                                                                                                                                                       self.k) / (self.context.pochhammer(1 + self.ell + self.epsilon - self.k,
                                                                                                                                                                                                                          self.k))
            else:
                raise RuntimeError(
...
class rational_approx_data_generic_dim:
    ...
    def __init__(self, context, cutoff, ell, Delta_1_2, Delta_3_4,
        ...
        self.approx_poles = []
        if is_correlator_multiple:
            self.poles = [poleData(1,
                                   x,
                                   ell,
                                   self.a,
                                   self.b,
                                   context) for x in range(1,
                                                           self.cutoff + 1)] + [poleData(2,
                                                                                         x,
                                                                                         ell,
                                                                                         self.a,
                                                                                         self.b,
                                                                                         context) for x in range(1,
                                                                                                                 (cutoff // 2) + 1)] + [poleData(3,
                                                                                                                                                 x,
                                                                                                                                                 ell,
                                                                                                                                                 self.a,
                                                                                                                                                 self.b,
        ...
        if approximate_poles:
            if ell == 0:
                ...
                unitarity_bound = ell + 2 * \
                    self.epsilon + context.field(0.5) ** 10
            dim_approx_base = len(self.poles)
            self.approx_column = lambda x: [1 / (unitarity_bound - x.polePosition()) ** i for i in range(
                1, dim_approx_base // 2 + 2)] + [(x.polePosition()) ** i for i in range(0, (dim_approx_base + 1) // 2 - 1)]
            self.approx_matrix = matrix(map(self.approx_column, self.poles))
            if is_correlator_multiple:
                self.approx_poles = [poleData(1,
                                              x,
                                              ell,
                                              self.a,
                                              self.b,
                                              context) for x in range(cutoff + 1,
                                                                      2 * cutoff + 1)] + [poleData(2,
                                                                                                   x,
                                                                                                   ell,
                                                                                                   self.a,
                                                                                                   self.b,
                                                                                                   context) for x in range((cutoff // 2) + 1,
                                                                                                                           2 * (cutoff // 2) + 1)] + [poleData(3,
                                                                                                                                                               x,
                                                                                                                                                               ell,
                                                                                                                                                               self.a,
                                                                                                                                                               self.b,
                                                                                                                                                               context) for x in range(min(ell,
                                                                                                                                                                                           cutoff // 2) + 1,
                                                                                                                                                                                       ell + 1)]
            else:
                self.approx_poles = [poleData(1,
                                              ...
                                              self.a,
                                              self.b,
                                              context) for x in range(2 * (cutoff // 2) + 2,
                                                                      2 * cutoff + 2,
                                                                      2)] + [poleData(2,
                                                                                      x,
                                                                                      ell,
                                                                                      ...
                                                                                      context) for x in range((cutoff // 2) + 1,
                                                                                                              2 * (cutoff // 2) + 1)] + [poleData(3,
                                                                                                                                                  ...
                                                                                                                                                  self.a,
                                                                                                                                                  self.b,
                                                                                                                                                  context) for x in range(2 * (min(ell,
                                                                                                                                                                                   cutoff) // 2) + 2,
                                                                                                                                                                          2 * (ell // 2) + 2,
                                                                                                                                                                          2)]
    ...
    def approx_h(self):
        res = self.context.h_asymptotic_form(self.S) * reduce(lambda y, w: y * w, [
    ...
    def prefactor(self):
        ...
        for i in range(1, len(__q)):
            if __q[i] == __q[i - 1]:
                ...
                __poles.append([__q[i], 1])
                __count = __count + 1
        return damped_rational(__poles, 4 *
                               self.context.rho, 1 /
                               (4 *
                                self.context.rho *
                                self.context.field(self.ell +
                                                   1)) *
                               self.context.field(4) ** (len(__chiral_poles)), self.context)
    ...
    def approx_g(self):
        ...
        for i in range(0, self.context.Lambda // 2 + 2):
            for j in range(i + 1, self.context.Lambda - i + 2):
                __zz_res.append(
                    (-left_contribution[i] * right_contribution[j] + left_contribution[j] * right_contribution[i]))
